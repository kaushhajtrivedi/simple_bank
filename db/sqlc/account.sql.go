...
const listenAccount = `-- name: ListenAccount :many
SELECT id, owner, balance, currency, created_at FROM accounts
WHERE owner = $1
ORDER BY id
LIMIT $2
...
type ListenAccountParams struct {
	Owner  string `json:"owner"`
	Limit  int32  `json:"limit"`
	Offset int32  `json:"offset"`
}

func (q *Queries) ListenAccount(ctx context.Context, arg ListenAccountParams) ([]Accounts, error) {
	rows, err := q.db.QueryContext(ctx, listenAccount, arg.Owner, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
